...
def app(symbols, start, end, threads, timeframe, folder, header, mergin):
    if start > end:
        return
    lock = threading.Lock()
    global day_counter
    ...
    def do_work(symbol, day, csv):
        ...
        try:
            csv.append(day, decompress(day, symbol, fetch_day(symbol, day)))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        files = {symbol: CSVDumper(symbol, timeframe, start, end, folder, mergin, header) for symbol in symbols}
