...
def add_hour(ticks):
    ...
    hour_delta = 0
    if ticks[0][0].weekday() == 6 or (ticks[0][0].day == 1 and ticks[0][0].month == 1 and ticks[0][0].year > 2010):
...
def normalize(day, symbol,  ticks):
    def norm(time, ask, bid, volume_ask, volume_bid):
        date = datetime(day.year, day.month, day.day) + timedelta(milliseconds=time)
        #if(re.match(r"[0-9][0-9][0-9][0-9][0-9][1-9]", str(ask))):
            #print("HIT: ", ask/1000, date)
        # date.replace(tzinfo=datetime.tzinfo("UTC"))
        return date, ask / 100000, bid / 100000, volume_ask, volume_bid 

    return add_hour(list(map(lambda x: norm(*x), ticks)))


def decompress(day,symbol, compressed_buffer):
    if compressed_buffer.nbytes == 0:
        return compressed_buffer
    return normalize(day, symbol, tokenize(decompress_lzma(compressed_buffer)))
